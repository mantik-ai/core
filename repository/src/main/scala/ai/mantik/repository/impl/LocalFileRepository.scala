...
import ai.mantik.repository.Errors.RepositoryError
...
import ai.mantik.repository.impl.LocalFileRepository.FileMetaData
...
class LocalFileRepository(config: Config, clock: Clock)(implicit val actorSystem: ActorSystem, ec: ExecutionContext, materializer: Materializer) extends FileRepositoryServer(config) {
  ...
  def removeTimeoutedFiles(): Unit = {
    val border = clock.instant().minus(cleanupTimeout.toSeconds, ChronoUnit.SECONDS)
