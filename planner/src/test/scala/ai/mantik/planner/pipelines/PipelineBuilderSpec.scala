...
class PipelineBuilderSpec extends TestBase {
  ...
  val algorithm1 = Algorithm(
    source = Source(DefinitionSource.Loaded("algo1"), PayloadSource.Loaded("file1", ContentTypes.MantikBundleContentType)),
    Mantikfile.pure(
      AlgorithmDefinition(
        stack = "stack1",
        `type` = FunctionType(
  ...
  it should "build pipelines" in {
    ...
    withClue("Algorithms which are loaded are using their real ids") {
      ...
      algo1.pipelineStep.asInstanceOf[PipelineStep.AlgorithmStep].algorithm shouldNot be('generated)
      val algo2 = pipeline.resolved.steps(1)
      algo2.pipelineStep.asInstanceOf[PipelineStep.AlgorithmStep].algorithm shouldBe 'generated
