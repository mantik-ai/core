...
class RemotePlanningContextImpl @Inject() (planningContext: PlanningContextService)(implicit akkaRuntime: AkkaRuntime) extends ComponentBase with PlanningContext {
  ...
  override def load(id: MantikId): MantikItem = {
    loadImpl(LoadItemRequest(
      mantikId = Conversions.encodeMantikId(id), pull = false
    ))
  }

  override def pull(id: MantikId): MantikItem = {
    loadImpl(LoadItemRequest(
      mantikId = Conversions.encodeMantikId(id), pull = true
    ))
  }
  ...
  override def execute[T](action: Action[T]): T = {
    ...
    val responseData: Future[ByteString] = StreamConversions.callMultiOut(Conversions.decodeErrors, planningContext.execute, request) {
      Sink.fold[ByteString, ExecuteActionResponse](ByteString.empty) {
        case (current, next) =>
  ...
  override def state(item: MantikItem): MantikItemState = {
    call {
      ...
      planningContext.state(request).map { response =>
        Conversions.decodeJsonItem[MantikItemState](response.stateJson, err => s"Could not decode MantikItemState: ${err}")
      }
    }
  }
