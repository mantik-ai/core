...
object Bridge {
  ...
  def fromMantikArtifacts(
    ...
    BuiltInItems.readBuiltInItem(name) match {
      ...
      case Some(other)     => ErrorCodes.MantikItemWrongType.throwIt(s"Expected bridge got ${other.mantikHeader.definition.kind}")
      case None            => // continue
    }
    ...
    val mantikHeader = bridgeArtifact.parsedMantikHeader.cast[BridgeDefinition].right.getOrElse {
      ErrorCodes.MantikItemWrongType.throwIt(s"${name} references a a${bridgeArtifact.parsedMantikHeader.definition.kind}, expected bridge")
    }
    val bridge = Bridge(
