...
class JoinParserSpec extends ParserTestBase {
  ...
  parseJoinTest(Seq("$0 JOIN $1 ON a = b", "$0 INNER JOIN $1 ON a = b"), AST.JoinNode(
  ...
  parseJoinTest(Seq("$0 CROSS JOIN $1"), AST.JoinNode(
    AST.AnonymousReference(0),
    AST.AnonymousReference(1),
    AST.JoinType.Inner,
  ...
  parseJoinTest(Seq("$0 JOIN $1 USING a", "$0 INNER JOIN $1 USING a"), AST.JoinNode(
    AST.AnonymousReference(0),
    AST.AnonymousReference(1),
    AST.JoinType.Inner,
  ...
  parseJoinTest(Seq("$0 LEFT JOIN $1 ON true", "$0 LEFT OUTER JOIN $1 ON true"), AST.JoinNode(
    AST.AnonymousReference(0),
    AST.AnonymousReference(1),
    AST.JoinType.Left,
    AST.JoinCondition.On(
  ...
  parseJoinTest(Seq("$0 RIGHT JOIN $1 USING a,b", "$0 RIGHT OUTER JOIN $1 USING a,b"), AST.JoinNode(
    AST.AnonymousReference(0),
    AST.AnonymousReference(1),
    AST.JoinType.Right,
    AST.JoinCondition.Using(
  ...
  parseJoinTest(Seq("$0 FULL OUTER JOIN $1 USING \"A\"", "$0 FULL JOIN $1 USING \"A\""), AST.JoinNode(
    AST.AnonymousReference(0),
    AST.AnonymousReference(1),
    AST.JoinType.Outer,
    AST.JoinCondition.Using(
