...
class UnionSpec extends TestBase {
  ...
  it should "run simple unions" in {
    Union(
      ...
      all = true
    ).run(bundle1, bundle3).forceRight shouldBe TabularBundle
      .build(
        bundle1.model
      )
      .row(1, "Hello")
  ...
  it should "run auto unions" in {
    autoUnion(bundle1, bundle2, true) shouldBe TabularBundle
      .build(
        TabularData(
          "x" -> FundamentalType.Int32,
    ...
    autoUnion(bundle1, bundle1, true) shouldBe TabularBundle
      .build(
        bundle1.model
      )
      .row(1, "Hello")
  ...
  it should "drop duplicates" in {
    autoUnion(bundle1, bundle3, false) shouldBe TabularBundle
      .build(
        bundle1.model
      )
      .row(1, "Hello")
  ...
  it should "run complex select based unions" in {
    sql("SELECT y FROM $0 WHERE x = 1 UNION SELECT y FROM $1 WHERE x = 3", bundle1, bundle3) shouldBe TabularBundle
      .build(
        TabularData(
          "y" -> FundamentalType.StringType
        )
  ...
  it should "allow multiple parameters" in {
    sql(
      "SELECT x FROM $0 UNION SELECT x FROM $1 UNION SELECT x FROM $2",
      bundle1,
      bundle2,
      bundle3
